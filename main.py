...
def scan_confirm_keyboard():
    kb=types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("🚀 Start FREE AI Scan", callback_data="scan:confirm"))
    kb.add(types.InlineKeyboardButton("✏️ Change Contract", callback_data="menu:analyse"))
    return kb
...
def build_start():
    return (
        "<b>MARSOF AI</b>\n"
        "<b>FREE AI-ASSISTED CRYPTO INTELLIGENCE</b>\n\n"
        "Analyze a cryptocurrency token with real-time market and security data — <b>FREE</b>.\n"
        ...
        "<b>FREE AI token analysis. No registration required.</b>"
    )
...
def build_detected_prompt(address, detections):
    ...
    return (
        ...
        "Ready to run the <b>FREE AI scan</b>?"
    )
...
if bot:
    ...
    def start(message):
        ...
        try:
            ...
            if prev and prev.get("message_id"):
                bot.delete_message(message.chat.id,prev.get("message_id"))
        except Exception:
            pass
        try:
            with open(LOGO_PATH, "rb") as logo:
                sent=bot.send_photo(message.chat.id, logo, caption=build_start(), parse_mode="HTML", reply_markup=main_keyboard())
        except Exception as exc:
            print(f"⚠️ logo send failed: {exc}")
            sent=bot.send_message(message.chat.id,build_start(),parse_mode="HTML",reply_markup=main_keyboard())
        bot._marsof_ui[message.chat.id]={"message_id":sent.message_id,"message_type":"photo","phase":"home","network":None,"result":None,"address":None,"detections":None}
    ...
    def text_handler(message):
        ...
        if not detections:
            bot.reply_to(message,"⚠️ MARSOF AI could not verify this contract on the supported networks.\n\nPlease check the address and try again. No network was guessed.")
            return
        state["detections"]=detections
        state["network"]=detections[0]["network"]
        state["address"]=address
        state["phase"]="awaiting_scan"
        bot._marsof_ui[message.chat.id]=state
        try:
            bot.edit_message_caption(caption=build_detected_prompt(address,detections),chat_id=message.chat.id,message_id=state["message_id"],parse_mode="HTML",reply_markup=scan_confirm_keyboard())
        except Exception as exc:
            print(f"⚠️ edit detected page failed: {exc}")
            try: bot.send_message(message.chat.id,build_detected_prompt(address,detections),parse_mode="HTML",reply_markup=scan_confirm_keyboard())
    ...
    def menu_callbacks(call):
        try:
            ...
            if action=="analyse":
                ...
                state.update({"phase":"awaiting_address","network":None,"result":None,"address":None,"detections":None,"message_id":call.message.message_id,"message_type":"photo"})
                bot._marsof_ui[call.message.chat.id]=state
                try:
                    bot.edit_message_caption(caption=build_analyze_prompt(),chat_id=call.message.chat.id,message_id=call.message.message_id,parse_mode="HTML",reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🏠 Home",callback_data="menu:home")))
                except Exception:
                    bot.edit_message_text(build_analyze_prompt(),call.message.chat.id,call.message.message_id,parse_mode="HTML",reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🏠 Home",callback_data="menu:home")))
            elif action=="home":
                state=bot._marsof_ui.get(call.message.chat.id,{})
                state.update({"phase":"home","network":None,"result":None,"address":None,"detections":None,"message_id":call.message.message_id,"message_type":"photo"})
                bot._marsof_ui[call.message.chat.id]=state
                try:
                    bot.edit_message_caption(caption=build_start(),chat_id=call.message.chat.id,message_id=call.message.message_id,parse_mode="HTML",reply_markup=main_keyboard())
                except Exception:
                    bot.edit_message_text(build_start(),call.message.chat.id,call.message.message_id,parse_mode="HTML",reply_markup=main_keyboard())
            bot.answer_callback_query(call.id)
        except Exception as exc:
            print(f"❌ menu callback: {exc}")
            bot.answer_callback_query(call.id,"Unable to open this page.")

    @bot.callback_query_handler(func=lambda call: call.data == "scan:confirm")
    def scan_confirm(call):
        try:
            state=getattr(bot,"_marsof_ui",{}).get(call.message.chat.id)
            if not state or state.get("phase")!="awaiting_scan" or not state.get("address") or not state.get("network"):
                bot.answer_callback_query(call.id,"Enter a contract address first."); return
            address=state["address"]; network=state["network"]
            bot.answer_callback_query(call.id,"Starting FREE AI scan…")
            bot.send_chat_action(call.message.chat.id,"typing")
            result=run_analysis(address,network)
            if not result:
                bot.edit_message_caption(caption="⚠️ <b>Analysis unavailable</b>\n\nMARSOF AI could not complete a verified scan. No values were fabricated.",chat_id=call.message.chat.id,message_id=call.message.message_id,parse_mode="HTML",reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("🔍 Analyze Another",callback_data="menu:analyse")))
                return
            state.update({"phase":"result","result":result,"network":network,"message_type":"photo"})
            bot._marsof_ui[call.message.chat.id]=state
            # Keep the logo as the visual identity of the single active UI message.
            try:
                bot.edit_message_caption(caption=build_home(result),chat_id=call.message.chat.id,message_id=call.message.message_id,parse_mode="HTML",reply_markup=keyboard(result))
            except Exception as exc:
                print(f"⚠️ result caption edit failed: {exc}")
                bot.edit_message_text(build_home(result),call.message.chat.id,call.message.message_id,parse_mode="HTML",reply_markup=keyboard(result))
        except requests.RequestException as exc:
            print(f"❌ data source error: {exc}")
            bot.answer_callback_query(call.id,"A live data source is temporarily unavailable.")
        except Exception as exc:
            print(f"❌ analysis error: {exc}")
            bot.answer_callback_query(call.id,"Technical analysis error.")
    ...
    def callbacks(call):
        try:
            ...
            action=call.data.split(":",1)[1]
            result=state["result"]
            if action=="comparison": text=comparison_text(result)
            else: text=section_text(result,action)
            bot.edit_message_caption(caption=text,chat_id=call.message.chat.id,message_id=call.message.message_id,parse_mode="HTML",reply_markup=keyboard(result))
            bot.answer_callback_query(call.id)
        except Exception as exc:
