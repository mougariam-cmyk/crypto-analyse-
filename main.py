...
if DATABASE_URL:
    ...
    # Render/Python 3.14: use psycopg v3 explicitly.
...
def parse_timestamp(value):
    """Parse Unix timestamps returned by APIs in seconds or milliseconds."""
    x = number(value)
    if x is None:
        return None
    # DEX Screener returns pairCreatedAt as Unix milliseconds.
    # Other sources may return Unix seconds, so support both without guessing.
...
def build_security(data):
    ...
    # Some contract-security items are nested under contract_scan in current GoPlus responses.
    contract_scan = data.get("contract_scan") if isinstance(data.get("contract_scan"), dict) else {}
    nested_security = {
        "privilege_withdraw": contract_scan.get("privilege_withdraw"),
        "approval_abuse": contract_scan.get("approval_abuse"),
        "withdraw_missing": contract_scan.get("withdraw_missing"),
        "selfdestruct": contract_scan.get("selfdestruct"),
    }
    ...
    for key, label in [
        ...
        if raw_value is None:
            raw_value = nested_security.get(key)
        if isinstance(raw_value, dict) and key == "metadata_modifiable":
            raw_value = raw_value.get("value", raw_value.get("status"))
        # GoPlus metadata_modifiable may use 2 to mean the function is available.
        if key == "metadata_modifiable" and str(raw_value).strip() == "2":
            v = True
        else:
            v = boolean(raw_value)
    ...
    # Owner / creator. GoPlus returns owner_address/owner_type at the
    # top level for Token Security API; contract_scan is a nested fallback.
    contract_scan = data.get("contract_scan") if isinstance(data.get("contract_scan"), dict) else {}
    scan_owner = contract_scan.get("owner") if isinstance(contract_scan.get("owner"), dict) else {}
    owner_address = data.get("owner_address") or scan_owner.get("owner_address")
    owner_type = data.get("owner_type") or scan_owner.get("owner_type")
...
def split_telegram_text(text_value, limit=3500):
    """Split long Telegram messages safely on line boundaries."""
...
def clear_extra_ui(chat_id, state):
    for message_id in state.get("extra_message_ids", []):
        try:
...
def render_section(chat_id, state, msg_text, edit_message_id):
    """Render a section even when it exceeds Telegram's message-size limit."""
    clear_extra_ui(chat_id, state)
    chunks = split_telegram_text(msg_text)
    ...
    for chunk in chunks[1:]:
        sent = bot.send_message(chat_id, chunk, parse_mode="Markdown")
        state["extra_message_ids"].append(sent.message_id)
...
def migrate_database():
    """Create tables and add any columns introduced by newer Step-1 versions.

    The production Render database may have been created by an older MARSOF build.
    SQLAlchemy create_all() does not add new columns to existing tables, so we do
    additive PostgreSQL migrations here. No existing data is deleted.
    """
    if not engine:
        return
    try:
        ...
        # Startup should remain alive so Render can expose the actual error.
