...
def fetch_dexscreener(address):
    ...
    try:
        ...
        print(f"❌ DEX Screener request failed for {target}: {exc}")
        raise

    pairs = payload.get("pairs") if isinstance(payload, dict) else None
    if not isinstance(pairs, list):
        print(f"⚠️ DEX Screener returned no pair list for {target}")
        return []

    matched = []
    for pair in pairs:
        if not isinstance(pair, dict) or pair.get("chainId") != "bsc":
            continue
    ...
    if matched:
        p = matched[0]
        print(f"✅ DEX Screener: {len(matched)} exact BSC pair(s) for {target}; selected {p.get('pairAddress')} liquidity={((p.get('liquidity') or {}).get('usd'))}")
    else:
...
def metric(label, value, status="DATA", detail=None, source="GoPlus"):
    # Missing source values are never counted as available DATA.
    if value is None and status == "DATA":
        status = "UNKNOWN"
    return {
        "label": label,
        "value": value,
...
def build_market(pair):
    if not pair:
        print("⚠️ MARKET: no exact BSC pair supplied by DEX Screener")
        return {
            "checks": [metric("DEX Market Pair", None, "UNKNOWN", "لم يوفر المصدر زوج BSC مطابقًا للعقد", "DEX Screener")],
            "data": {
                "pair": None, "price": None, "market_cap": None, "fdv": None,
                "liquidity": None, "price_native": None, "volume_24h": None,
                "buys_24h": None, "sells_24h": None, "price_change_24h": None,
                "liq_mc": None, "liq_fdv": None, "vol_liq": None, "vol_mc": None,
                "fdv_mc": None, "periods": {},
            },
            "missing": ["DEX market data"],
        }
...
def run_analysis(address):
    ...
    if not security:
        print(f"⚠️ GoPlus: no exact security result for {target}")
        return None

    pairs = fetch_dexscreener(address)
    selected = choose_market(pairs)
    market = build_market(selected)
    security_checks = build_security(security)
    classification = classify_all(security_checks, market["checks"])

    # Compare BEFORE saving this scan, so this is genuinely the previous scan.
    ...
    print(f"📊 Analysis {target}: market_pair={bool(selected)} price={market['data'].get('price')} liquidity={market['data'].get('liquidity')} previous_scan={comparison.get('available')}")
    return result
